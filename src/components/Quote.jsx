    ...
    <div id="Quote">
      {quotes.map((quote, index) => (
        <div className="m-24" key={index}>
          <div className="w-full p-4 text-center bg-gray-200 border border-gray-200 rounded-lg shadow sm:p-8 dark:bg-gray-800 dark:border-gray-700">
            <h5 className="mb-2 text-2xl font-bold text-gray-900 dark:text-white">{quote.text}</h5>
            <p className="mb-5 text-base text-gray-500 sm:text-lg dark:text-gray-600">{quote.author}</p>
          </div>
        </div>
      ))}
    </div>
